...
var UploadcareError = class extends Error {
};
var NetworkError = class _NetworkError extends UploadcareError {
...
var CancelError = class _CancelError extends UploadcareError {
...
var UploadError = class _UploadError extends UploadcareError {
...
function base(file, { publicKey, fileName, contentType, baseURL = defaultSettings.baseURL, secureSignature, secureExpire, store, signal, onProgress, source = "local", integration, userAgent, retryThrottledRequestMaxTimes = defaultSettings.retryThrottledRequestMaxTimes, retryNetworkErrorMaxTimes = defaultSettings.retryNetworkErrorMaxTimes, metadata }) {
  return retryIfFailed(() => request({
    ...
    if ("error" in response2) {
      throw new UploadError(response2.error.content, response2.error.errorCode, request2, response2, headers);
    } else {
      return response2;
    }
...
function multipartStart(size, { publicKey, contentType, fileName, multipartChunkSize = defaultSettings.multipartChunkSize, baseURL = "", secureSignature, secureExpire, store, signal, source = "local", integration, userAgent, retryThrottledRequestMaxTimes = defaultSettings.retryThrottledRequestMaxTimes, retryNetworkErrorMaxTimes = defaultSettings.retryNetworkErrorMaxTimes, metadata }) {
  return retryIfFailed(() => request({
    ...
    if ("error" in response2) {
      ...
      response2.parts = Object.keys(response2.parts).map((key) => response2.parts[Number(key)]);
      return response2;
    }
  }), { retryThrottledRequestMaxTimes, retryNetworkErrorMaxTimes });
}
...
var UploadcareFile = class {
  constructor(fileInfo, { baseCDN = defaultSettings.baseCDN, fileName } = {}) {
    ...
    __publicField(this, "s3Url");
    __publicField(this, "originalFilename");
    __publicField(this, "imageInfo");
    __publicField(this, "videoInfo");
    __publicField(this, "contentInfo");
    __publicField(this, "metadata");
    __publicField(this, "s3Bucket");
    __publicField(this, "defaultEffects", null);
...
var Events = class {
  ...
  emit(event, data) {
    var _a;
    (_a = this.events[event]) == null ? void 0 : _a.forEach((fn) => fn(data));
  }
  on(event, callback) {
...
var Pusher = class {
  ...
  send(event, data) {
    ...
    (_a = this.ws) == null ? void 0 : _a.send(str);
  }
  ...
  return poll({
    check: (signal2) => fromUrlStatus(token, {
      ...
      switch (response2.status) {
        case Status.Error: {
          return new UploadError(response2.error, response2.errorCode);
        }
        case Status.Waiting: {
          return false;
        }
...
var UploadcareGroup = class {
  constructor(groupInfo2, { baseCDN = defaultSettings.baseCDN } = {}) {
    __publicField(this, "uuid");
    ...
    this.uuid = groupInfo2.id;
    this.filesCount = groupInfo2.filesCount;
    const groupFiles = groupInfo2.files.filter(Boolean);
    this.totalSize = Object.values(groupFiles).reduce((acc, file) => acc + file.size, 0);
    this.isStored = !!groupInfo2.datetimeStored;
    this.isImage = !!Object.values(groupFiles).filter((file) => file.isImage).length;
    this.cdnUrl = groupInfo2.cdnUrl;
    this.files = groupFiles.map((fileInfo) => new UploadcareFile(fileInfo, { baseCDN }));
    this.createdAt = groupInfo2.datetimeCreated;
    this.storedAt = groupInfo2.datetimeStored;
  }
};
...
function uploadFileGroup(data, { publicKey, fileName, baseURL = defaultSettings.baseURL, secureSignature, secureExpire, store, signal, onProgress, source, integration, userAgent, retryThrottledRequestMaxTimes, retryNetworkErrorMaxTimes, contentType, multipartChunkSize = defaultSettings.multipartChunkSize, baseCDN = defaultSettings.baseCDN, checkForUrlDuplicates, saveUrlForRecurrentUploads, jsonpCallback }) {
  ...
  return Promise.all(data.map((file, index) => {
    ...
    return group(uuids, {
      ...
      retryThrottledRequestMaxTimes,
      retryNetworkErrorMaxTimes
    }).then((groupInfo2) => new UploadcareGroup(groupInfo2, { baseCDN })).then((group2) => {
      onProgress && onProgress({ isComputable: true, value: 1 });
      return group2;
    });
  });
...
var Queue = class {
  constructor(concurrency) {
    __publicField(this, "_concurrency", 1);
    __publicField(this, "_pending", []);
    __publicField(this, "_running", 0);
    __publicField(this, "_resolvers", /* @__PURE__ */ new Map());
    __publicField(this, "_rejectors", /* @__PURE__ */ new Map());
...
export {
  ...
  fromUrl,
  fromUrlStatus,
  getUserAgent$1 as getUserAgent,
  group,
  groupInfo,
  info,
  isReadyPoll,
...
//# sourceMappingURL=@uploadcare_upload-client.js.map
